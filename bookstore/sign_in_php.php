<?php 
    ...
    else if($conn)
    {
        ...
        // echo "working";
        if($num_of_result == 0 && $user_pass == $cuser_pass && $key=="cse"){
            $pre_statemnt->close();
            $insert = "INSERT INTO sign_up (u_name, u_email, u_password) VALUES(?, ?, ?)";
            $pre_statemnt = $conn->prepare($insert);
            $pre_statemnt->bind_param("sss", $user_name, $user_email, $user_pass);
            $pre_statemnt->execute();
